use crate::{bme280::BME280Sensor, lora::LoRa, BME280Config, Config, LoRaConfig};
...
fn post_lora(lora_config: &LoRaConfig) -> Result<()> {
    if !Path::new(&lora_config.spi_config.spidev_path).exists() {
        ...
        error!("[ ERR ] SPI POST");
        return Err(anyhow!("Spidev path {} doesn't exist!", &lora_config.spi_config.spidev_path));
    }

    let mut lora = LoRa::from_config(lora_config)?;
    let mut mode = 0;
...
fn post_bme(bme_config: &BME280Config) -> Result<()> {
    if !Path::new(&bme_config.i2c_bus_path).exists() {
        ...
        return Err(anyhow!("i2c bus path {} doesn't exist!", &bme_config.i2c_bus_path));
    }
...
fn post_mqtt() -> Result<()> {
    ...
    let payload = [1; 24];
    match ping(ip, Some(Duration::from_secs(1)), Some(128), None, None, Some(&payload)) {
        Ok(_) => {
